...
class Robot: public IterativeRobot  // FRC3538 IRI 2016
{
	RobotDrive myRobot,Robot8; // robot 10 wheel drive system
	Victor Catapult,Intake,Wedge;  //motor to control catapult
	...
	Solenoid *AutoSolenoid  = new Solenoid(3);
	Timer FwdTime, LoopTime, AutoDelayTime;
	bool CatDown, CatActive, ArmExtend;
...
public:
	Robot() :
		...
		Robot8(3,2),
		Catapult(9),Intake(7),Wedge(6),		// PWM 9 for catapult control
		...
		CatDown(false), CatActive(true),ArmExtend(false),AutoLow(false),AutoHi(false),AutoLowLong(false),
		AutoCatDown(false),AutoFireLatch(false), AutoDelayChk(false),AutoDelayCmp(false),
...
//			AutoDelayTime.Reset();
		...
		if (!CompleteCourtYard and (DefSelected == autoDef1) and CompleteOuter){		//Position 1 code (low bar)
			...
			//AutoSolenoid->Set(true);	// verify stage2 temp use only remove for final
			}
			if (!CYstep1 and !ChkPt1){
				...
				FwdTime.Start();
				CYstep1=true;
				//AutoSolenoid->Set(false);	// verify stage2 temp use only remove for final
				}

			ChkPt1 = false;
			if (CYstep1 and !CYstep2){
			ChkPt1 = DriveFwd(DefSelected, autoDef1, 0.0,-0.75,false);		//Drive forward 0.75 for 0.5 sec
			//Arm2Solenoid->Set(true);	// verify stage2 temp use only remove for final
			}
			if (!ChkPt1 and CYstep1){
				...
				//Arm2Solenoid->Set(false);	// verify stage2 temp use only remove for final
				}
	...
	void TeleopInit()
	{
		...
		Arm1Solenoid->Set(false);  Arm2Solenoid->Set(false);
	...
	void TeleopPeriodic()
	{
		...
		bool Fire = Ybutton or Abutton;
		bool Latched = DiIn7.Get();		// get Latch switch
		bool CatDwn1 = DiIn9.Get();      // get Digital value from channel 9
		bool Override = LMbumper;
		bool CatDwn2 = DiIn8.Get();      // get Digital value from channel 8
		CatDown = !CatDwn1 or !CatDwn2; // Latch CatDown if one switch is false
		float LatchSpeed;
		if (!CatDown and FwdTime.Get() > 2.0 ){	//Catapult not down drive forward
			 LatchSpeed = 1.0;
			 CatActive=true;}
		//Not Latched and Not Tension and both bumpers drive forward
		else if (!CatDown and Override){
			 LatchSpeed = 1.0;
			 FwdTime.Start();
			 CatActive=true;}    //Start timer to allow tension
		else if (!CatDown)
			CatActive=true;		//Extend arm of Catapult not down
		//else if (CatDown and Fire)
			//CatActive=true;		//Extend arm before Catapult fires
		else if (CatDown and Fire){
			LatchSpeed = -1.0;
			FwdTime.Stop();
			FwdTime.Reset();
			FwdTime.Start();    //Start timer all time to shoot
			CatActive=true;
		}
		else{
			LatchSpeed = 0.0;
		...
		if (Xbutton)
			...
			Wedge.Set(0.0);
		//bool FwArm = DiIn6.Get();      // get Digital value from channel 6 Arm Forward
		//bool RwArm = DiIn5.Get();      // get Digital value from channel 5 Arm Rear
		...
		float InTrig;
		if (RmTrig>0)			//Combine Left and Right Triger
			InTrig = RmTrig;	// Right trigger Positive
		else if (LmTrig>0)
			InTrig = -LmTrig;  // Left trigger Negative
		else
			InTrig=0;
		...
		//if (InTrig > 0 and FwArm){
		//	Arm.Set(InTrig);
			//Intake.Set(0.5);
		//}
		//else if (InTrig > 0 and !FwArm){
		//	Arm.Set(0.0);
			//Intake.Set(0.5);
		//}
		//else if (InTrig < 0 and RwArm){
		//	Arm.Set(InTrig);
		//	//Intake.Set(-0.5);
		//}
		//else if (InTrig < 0 and !RwArm){
		...
		SmartDashboard::PutBoolean("Launch", Fire); //Release catapult
		SmartDashboard::PutBoolean("Cat Down1", CatDwn1); // Display Latch1
		SmartDashboard::PutBoolean("Override", Override);
		SmartDashboard::PutBoolean("Cat Down2", CatDwn2);
		SmartDashboard::PutBoolean("Cat Down", CatDown);
		SmartDashboard::PutBoolean("Latched", Latched);
		SmartDashboard::PutNumber("Forward Time", FwdTime.Get());
		SmartDashboard::PutNumber("Latch Speed", LatchSpeed);
		SmartDashboard::PutNumber("Intake Trigger", InTrig); // Display trigger value
